...
//   FILE *file = fopen("port_stext.txt", "w");
//   if (file == NULL)
//   {
...
int createSocket()
{
  ...
  serv_addr.sin_port = htons(STEXT_PORT);
...
// replacing string with given string
...
{
  system("find ./stext -type f \\( -name '*.txt' \\) -print0 | tar --null -cvf ./stext/txtFiles.tar --files-from=-");
}
...
void handle_request(int client_sock)
{
  ...
  replaceSubstring(client_string, "~smain", "./stext");
  ...
  else if (strcmp(command, "rmfile") == 0)
  {
    ...
    printf("dest_path for remove:: %s\n", dest_path);

    handleRemoveFile(client_sock, dest_path);
  }
  // Creating tar file of all text files
  else if (strcmp(command, "dtar") == 0)
  {
    ...
    sendTarFile(client_sock, "./stext/txtFiles.tar");
  }
  ...
  {
    printf("Invalid command run at Stext server\n");
  }
}

int main()
{
  // set stext port number random and store in file
  // setPort();
